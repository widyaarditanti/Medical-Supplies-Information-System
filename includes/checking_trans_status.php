		...
		if(mysqli_stmt_prepare($stmt,$sql)){
			...
			if(mysqli_stmt_prepare($stmt,$sql)){
				...
				while($row = mysqli_fetch_assoc($result)) {
					...
					else if($row['approval'] == "denied"){
						$statusDenied = 1;
					}
					else if($row['approval'] =="no"){
						$StatusNo = 1;
					}
					echo $row['approval'];
				}
				echo $statusYes;
				echo $statusNo;
				if($statusYes == 1 && $statusNo == 0){
					$status = "on going";
				}
				else if($statusYes == 1 && $statusNo == 1){
					$status = "on going";
				}
				else if($statusNo == 1 && $statusYes == 0 && $statusDenied == 0 ){
					$status = "waiting";
				}
				else if($statusDenied == 1 && $statusYes == 1 && $statusNo == 1){
					$status = "on going";
				}
				else if($statusDenied == 1 && $statusYes == 1 && $statusNo == 0){
					$status = "on going";
				}
				else if($statusDenied == 1 && $statusYes == 0 && $statusNo == 1){
					$status = "waiting"; //?
				}
